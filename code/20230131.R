...
seed_num <- 4605
bg_colour <- "#33312E"
palette <- c("#EA481F", "#E9AE49", "#662C91", "#17A398", "#C1EF1A")
length_out <- 700
...
ggsave(
  here::here("img/ingredients/20230131.png"), last_plot(),
  width = 12, height = 9, units = "in", dpi = 600)

# Switch to {magick} workflow --------------------------------------------------

img <- image_read(here::here("img/ingredients/20230131.png"))

img_implode <- img %>%
  image_implode(factor = -0.7) %>%
  image_morphology(
    method = "Dilate", kernel = "Disk", iterations = 8) %>%
...
image_write(
  img_implode, path = here::here("img/20230131.png"))
